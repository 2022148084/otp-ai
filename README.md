...
### 설정 주의사항
# min_containers=0 (안심 설정)

의미: 요청이 없으면 서버 개수를 0개로 줄입니다.
가이드: 작업을 마치고 터미널을 꺼도, 서버가 알아서 꺼지므로 요금 걱정 안 하셔도 됩니다. (Stop 버튼 누를 필요 없음)




# scaledown_window=100 (대기 시간)

의미: 작업이 끝나도 100초(약 1분 30초) 동안은 끄지 않고 대기합니다.
연속으로 modal run을 할 때 매번 다시 켜지는(Cold Start) 시간을 줄이기 위한 설정입니다.
테스트가 끝나도 약 1분간은 'Running' 상태로 보일 수 있으나, 정상이니 놔두시면 됩니다.
튜닝 중에는 이 값을 10~30 정도로 줄여서 테스트해도 무방합니다.


### 작업 흐름

1. `deploy.py` 수정
2. `modal run deploy.py`로 동영상 결과 확인, `modal run deploy.py --file-path test1.jpg`로 이미지 테스팅
3. 만족스러우면 깃허브에 브랜치 만들고 push
